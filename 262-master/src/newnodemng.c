...
void quit(ListNode *head, historyBuffer *hsb) {
    ...
    while (current != NULL) {
        ...
        // Free file contents if not NULL
        if (temp->file_contents) {
            free(temp->file_contents);
            temp->file_contents = NULL;
        }
    ...
    for (int i = 0; i < hsb->count; i++) {
        if (hsb->commands[i]) {
            free(hsb->commands[i]);
            hsb->commands[i] = NULL; // Avoid dangling pointers
        }
    }
...
int add_to_history(historyBuffer *hsb, cla *s) {
    ...
    // Free the old command at the tail, if it exists
    if (hsb->commands[hsb->tail] != NULL) {
        free(hsb->commands[hsb->tail]);
        hsb->commands[hsb->tail] = NULL; // Avoid dangling pointer
    }
    ...
    // Update tail and handle wrapping
...
int main() {
    ...
    quit(head, &hsb);  // Ensure cleanup before exiting
    return 0;
}
